...
/*   Updated: 2019/11/13 15:34:28 by bboisset         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int    store_rest(char *temp, char **rest)
{
    ...
    while (temp[i] != '\0')
        if (temp[i++] == '\n')
        {
            ...
            {
                free(*rest);
                *rest = (ft_substr_gnl(temp, i, temp_len));
                if (!rest)
                    return (0);
                return (1);
            }
...
int    read_line(int const fd, char **line, char **rest)
{
           ...
           ((res = read(fd, buffer, BUFFER_SIZE)) != 0))
    {
        ...
        buffer[res] = '\0';
        if (!(temp = ft_strjoin_gnl(temp, buffer)))
            return (exit_fct(&temp, &buffer));
    }
    *line = ft_substr_gnl(temp, 0, end_of_line(temp) - 1);
