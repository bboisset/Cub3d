...
/*   Updated: 2020/08/02 15:36:15 by bboisset         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (D_CONFIG->map[(int)(D_CAM->pos.x - D_CAM->fov.x * moveSpeedSec)][(int)D_CAM->pos.y] - 48 == 0)
		D_CAM->pos.x -= D_CAM->fov.x * moveSpeed;
	if (D_CONFIG->map[(int)D_CAM->pos.x][(int)(D_CAM->pos.y - D_CAM->fov.y * moveSpeedSec)] - 48 == 0)
		D_CAM->pos.y -= D_CAM->fov.y * moveSpeed;
	mlx_destroy_image(D_DATA->mlx_ptr, D_DATA->mlx_img);//clean image instead
	D_DATA->mlx_img = mlx_new_image(D_DATA->mlx_ptr, D_CONFIG->resolution.x, D_CONFIG->resolution.y);
	reload_scene(display_config, 1);
}
